...
float32 omega_filter[3] = {0, 0, 0};
float32 accel_filter[3] = {0, 0, 0};

static const uint8_t list_len = 5;
float32 omega_list[3][list_len] = {0};
float32 accel_list[3][list_len] = {0};

static const float read_IMU_frequency = 1500;  // Hz
...
static const float read_foot_contact_frequency = 1500;  // Hz
...
static const float send_data_frequency = 1000;  // Hz
...
float32 moving_average(float32 *data_list, float32 new_data);
...
void setup_IMU(void)
{
  ...
  // set gyroscope for 1.66 kHz, 500dps (0b 1000 01 0 0)
  uint8_t CTRL2_G = 0b10000100;
  i2c_reg_write(i2c_IMU, ISM330DHCX_ADDR, REG_CTRL2_G, &CTRL2_G, 1);
  // enable gyroscope LPF1 (0b 0 0 0 0 0 0 1 0)
  // uint8_t CTRL4_C = 0b00000010;
  // i2c_reg_write(i2c_IMU, ISM330DHCX_ADDR, REG_CTRL4_C, &CTRL4_C, 1);
  // gyroscope LPF1 bandwidth 12.6 (0b 0 0 0 0 0 111)
  // uint8_t CTRL6_C = 0b00000111;
  ...
  // set accelerometer for 1.66 kHz, 4g, data from LPF1 (0b 1000 10 0 0)
  uint8_t CTRL1_XL = 0b10001000;
  i2c_reg_write(i2c_IMU, ISM330DHCX_ADDR, REG_CTRL1_XL, &CTRL1_XL, 1);
  // accelerometer LPF2 cutoff ODR/200 (0b 101 0 0 0 0 0)
...
bool read_IMU(void)
{
  ...
  if (i2c_reg_read(i2c_IMU, ISM330DHCX_ADDR, REG_OUTX_L_G, data, 12))
  {
    ...
    for (int i = 0; i < 3; i++)
    {
      ...
      omega_filter[i] = moving_average(omega_list[i], omega[i]);
      accel_filter[i] = moving_average(accel_list[i], accel[i]);
    }
...
float32 moving_average(float32 *data_list, float32 new_data)
{
  ...
  data_list[list_len-1] = new_data;

  float32 data_avg = {0};
  for (int i = 0; i < list_len; i++)
  {
    data_avg.floating += data_list[i].floating;
  }
  data_avg.floating /= list_len;
  return data_avg;
}
...
void send_data(bool is_raw)
{
  ...
  else
  {
    ...
    {
      int idx = i * 4;
      send_data_packet[idx + 2] = accel_filter[i].binary[0];
      send_data_packet[idx + 3] = accel_filter[i].binary[1];
      send_data_packet[idx + 4] = accel_filter[i].binary[2];
      send_data_packet[idx + 5] = accel_filter[i].binary[3];
    
      send_data_packet[idx + 14] = omega_filter[i].binary[0];
      send_data_packet[idx + 15] = omega_filter[i].binary[1];
      send_data_packet[idx + 16] = omega_filter[i].binary[2];
      send_data_packet[idx + 17] = omega_filter[i].binary[3];
    }
  }
